 ...
 */
public class MyDebugOfKafkaSpout<K, V> extends BaseRichSpout {
    private static final Logger LOG = LoggerFactory.getLogger(MyDebugOfKafkaSpout.class);
    private static final Comparator<KafkaSpoutMessageId> OFFSET_COMPARATOR = new MyDebugOfKafkaSpout.OffsetComparator();
    ...
    private transient KafkaSpoutConfig.FirstPollOffsetStrategy firstPollOffsetStrategy;  // Strategy to determine the fetch offset of the first realized by the spout upon activation
    private transient KafkaSpoutRetryService retryService;              // Class that has the logic to handle tuple failure
    private transient MyDebugOfKafkaSpout.Timer commitTimer;                                // timer == null for auto commit mode
    ...
    private transient Map<TopicPartition, MyDebugOfKafkaSpout.OffsetEntry> acked;           // Tuples that were successfully acked. These tuples will be committed periodically when the commit timer expires, after consumer rebalance, or on close/deactivate
    ...
    public MyDebugOfKafkaSpout(KafkaSpoutConfig<K, V> kafkaSpoutConfig) {
        this.kafkaSpoutConfig = kafkaSpoutConfig;                 // Pass in configuration
        this.kafkaSpoutStreams = kafkaSpoutConfig.getKafkaSpoutStreams();
    }

    private transient MyDebugOfKafkaSpout.Timer debugTimer1;
    boolean isTimer1 = false;

    private transient MyDebugOfKafkaSpout.Timer debugTimer2;                                // timer == null for auto commit mode
    ...
    public void open(Map conf, TopologyContext context, SpoutOutputCollector collector) {
        ...
        consumerAutoCommitMode = false; //kafkaSpoutConfig.isConsumerAutoCommitMode();
        ...
        if (!consumerAutoCommitMode) {     // If it is auto commit, no need to commit offsets manually
            commitTimer = new MyDebugOfKafkaSpout.Timer(500, kafkaSpoutConfig.getOffsetsCommitPeriodMs(), TimeUnit.MILLISECONDS);
        }

        //debugTimer1 = new MyDebugOfKafkaSpout.Timer(1000*60*1, 1000*60*1, TimeUnit.MILLISECONDS);
        //debugTimer2 = new MyDebugOfKafkaSpout.Timer(1000*60*5, 1000*60*5, TimeUnit.MILLISECONDS);
    ...
    private void setAcked(TopicPartition tp, long fetchOffset) {
        ...
        if (!consumerAutoCommitMode && !acked.containsKey(tp)) {
            acked.put(tp, new MyDebugOfKafkaSpout.OffsetEntry(tp, fetchOffset));
        }
    }
    ...
    public void nextTuple() {
        boolean cmt = false, pol = false, waitemt = false;
//        if (debugTimer1.isExpiredResetOnTrue() && !isTimer1) {
//            isTimer1 = true;
//
//            numUncommittedOffsets = kafkaSpoutConfig.getMaxUncommittedOffsets() + 2000;
//            acked.clear();
...
//        }
//
//        if (debugTimer2.isExpiredResetOnTrue() && !isTimer2) {
//            isTimer2 = true;
//
//            numUncommittedOffsets = 0;
//        }
        ...
        LOG.info(" nextTuple:commit:"+cmt+"; poll:"+pol+"; waitingToEmit:"+waitemt+" ");
    }
    ...
    private boolean poll() {
        LOG.info(Thread.currentThread().getName()+":numUncommittedOffsets : "+numUncommittedOffsets);
        return !waitingToEmit() && numUncommittedOffsets < kafkaSpoutConfig.getMaxUncommittedOffsets();
    }

    private boolean waitingToEmit() {
        LOG.info("waitingToEmit : "+(waitingToEmit != null));
    ...
    private void commitOffsetsForAckedTuples() {
        ...
        for (Map.Entry<TopicPartition, MyDebugOfKafkaSpout.OffsetEntry> tpOffset : acked.entrySet()) {
        ...
        if (!nextCommitOffsets.isEmpty()) {
            ...
            for (Map.Entry<TopicPartition, MyDebugOfKafkaSpout.OffsetEntry> tpOffset : acked.entrySet()) {
                final MyDebugOfKafkaSpout.OffsetEntry offsetEntry = tpOffset.getValue();
    ...
    private void subscribeKafkaConsumer() {
        ...
        if (kafkaSpoutStreams instanceof KafkaSpoutStreamsNamedTopics) {
            ...
            kafkaConsumer.subscribe(topics, new MyDebugOfKafkaSpout.KafkaSpoutConsumerRebalanceListener());
            ...
            kafkaConsumer.subscribe(pattern, new MyDebugOfKafkaSpout.KafkaSpoutConsumerRebalanceListener());
